...
abstract class elFinderVolumeDriver {
	...
	public function rename($hash, $name) {
		$params = require(__DIR__ . '/../../../livefactory/config/main-local.php');
		$link = mysql_connect('localhost',$params["components"]["db"]["username"],$params["components"]["db"]["password"]);
		mysql_select_db(end(explode('dbname=',$params["components"]["db"]["dsn"])),$link);
	...
	public function upload($fp, $dst, $name, $tmpname) {
		...
		session_start();
		$extension =strrchr($name,'.');
		$path1 = $this->decode($dst);
		
		
		$dir  = $this->_dirname($path1);
		$sql = "insert into tbl_file (file_name,file_title,file_path,entity_id,entity_type,added_at,added_by_user_id) values('$name','$name','$dstpath','".$_SESSION['entity_id']."','".$_SESSION['entity_type']."','".time()."','".$_SESSION['user_id']."')";
		mysql_query($sql);
		$id = mysql_insert_id();
		$name = $id.$extension;
		$this->clearcache();
