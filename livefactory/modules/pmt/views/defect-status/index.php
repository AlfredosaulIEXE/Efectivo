...
<?php
	if(!empty($_GET['added'])){?>
		<div class="alert alert-success"><?=Yii::t('app', 'Defect Status is Added')?> </div>
<?php	}
?>
...
//            'modified_at', 
            ...
            [
				...
				'defaultValue' => function ($url, $model) {
					if(\livefactory\models\DefaultValueModule::checkDefaultValue('defect_status',$model->id)){
						return Html::a('<span class="fa fa-eraser"></span>', Yii::$app->urlManager->createUrl(['pmt/defect-status/index','del_id' => $model->id]), [
                                                    'title' => Yii::t('app', 'Delete Default'),
                                                  ]);
					}else{
						return Html::a('<span class="fa fa-tag"></span>', Yii::$app->urlManager->createUrl(['pmt/defect-status/index','id' => $model->id]), [
