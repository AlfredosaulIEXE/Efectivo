...
$(document).ready(function(e) {
	...
	$(document).on('click','.addTiming',function(){
			...
			$('#start_time').change(function(){
				...
				$.post('<?=$_SESSION['base_url']?>?r=pmt/defect/ajax-defect&id=<?=$_REQUEST['id']; ?>&start_time='+escape($(this).val())+'&eid=<?=$_REQUEST['time_entry_id']; ?>',function(result){
			...
			$('#end_time').change(function(){
					...
					$.post('<?=$_SESSION['base_url']?>?r=pmt/defect/ajax-defect-time-date-validation&start_time='+escape(start_time)+'&end_time='+escape(end_time),function(result){
						Remove_Error($('#end_time'));
					if(result=='yes'){
			...
			$('#timingSubmit').click(function(event){
					...
					if(endTime > nowD){
						...
						Add_Error($('#end_time'),"<?=Yii::t ('app','You can not use Future Time')?>");
...
<div class="modal fade defect_timing">
  ...
  <div class="modal-dialog  modal-lg">
    ...
    <div class="modal-content">
        ...
        <h4 class="modal-title"><?=Yii::t('app','Defect Timing')?></h4>
      ...
      <div class="modal-body">
			  ...
			  <form  action="" method="post" enctype="multipart/form-data" name="defect_time" id="defect_time">
    ...
    <input type="hidden" name="_csrf" value="<?php echo $this->renderDynamic('return Yii::$app->request->csrfToken;'); ?>">
    		...
    		<?php if($_GET['time_entry_id']){?>
            	...
            	<input type="hidden" name="defect_timing_edit" value="true">

            <?php }else{?>

    			<input type="hidden" name="defect_timing_add" value="true">
