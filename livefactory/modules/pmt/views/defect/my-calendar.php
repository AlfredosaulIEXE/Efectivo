...
foreach($dataProvider as $row){
	if(($row['expected_end_datetime']) < time() and $row['defect_status_id'] !=DefectStatus::_COMPLETED){
		$color='#F00';	
	}else if($row['defect_status_id'] ==DefectStatus::_COMPLETED){
		$color='#090';	
	}else{
		$color='#F90';
	}
	...
	$jSon.=$coma."{'id':'".$row['id']."','color':'".$color."','title':'".addslashes($row['defect_id'])."','start':'".$row['expected_start_datetime']."','end':'".$row['expected_end_datetime']."','url':'index.php?r=pmt/defect/defect-view&id=".$row['id']."'}";
	$coma=",";
 } 
$jSon.="]"; 
//echo $jSon;
