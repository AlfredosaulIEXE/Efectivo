...
$this->title=Yii::t ('app','Task Closed Report');
...
<form action="index.php?r=pmt/task/task-closed-reports" method="post">
	...
	<div class="panel-heading">
    	<h3 class="panel-title"><?=$this->title?>
        	<div class="pull-right">
            	<a href="index.php?r=pmt/task/task-closed-reports&date=last_month" class="btn btn-default btn-xs">
                
<?=Yii::t ( 'app','Last Month')?>
                	
                </a>
                <a href="index.php?r=pmt/task/task-closed-reports&date=this_month" class="btn btn-default btn-xs">
<?=Yii::t ( 'app','This Month')?>
                	
                </a>
                <a href="index.php?r=pmt/task/task-closed-reports&date=this_week" class="btn btn-default btn-xs">
<?=Yii::t ( 'app','This Week')?>
                	
                </a>
                <a href="index.php?r=pmt/task/task-closed-reports&date=yesterday" class="btn btn-default btn-xs">
<?=Yii::t ( 'app','Yesterday')?>
                	
                </a>
                <a href="index.php?r=pmt/task/task-closed-reports&date=today" class="btn btn-default btn-xs">
<?=Yii::t ( 'app','Today')?>
	...
	if(!in_array($_GET['date'],$filter)){
        ...
        <div class="row">
            <div class="col-sm-6">
                <div class="panel panel-info">
                    <div class="panel-heading">
                        <h3 class="panel-title"><?=Yii::t ( 'app','Task Closed Report Table')?></h3>
                    </div>
                    <div class="panel-body">
            ...
            <div class="col-sm-6">
                <div class="panel panel-info">
                    <div class="panel-heading">
                        <h3 class="panel-title"><?=Yii::t ( 'app','Task Closed Report Chart')?></h3>
                    </div>
                    <div class="panel-body">
                        <div id="taskClosed" style="height:350px"></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        foreach($dataProvider as $row){
            ...
            foreach($dates as $date){
                    ...
                    $c='';
                    foreach($users as $user){
                        $count=$userCount[$date][$user]==''?0:floatval($userCount[$date][$user]);
                        $jSon.= $c.$count;
                        $c=',';
                    }
                    $jSon .= "]";
        ...
        <script type="text/javascript">
            ...
            var chart = new google.visualization.ColumnChart(document.getElementById('taskClosed'));
        ...
        <?php 
				...
				foreach($dataProvider as $row){
					...
					if(!in_array($row['first_name'],$users)){
						...
						$userCount[$time][$row['first_name']]= $row['taskcount'];
				}
				$jSon = "[['Date',";
				$cm='';
					foreach($users as $user){
