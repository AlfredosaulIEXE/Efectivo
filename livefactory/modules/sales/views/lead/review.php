            ...
            'columns' => [
                ...
                [
                    ...
                    'value' => function ($model, $key, $index, $widget) {
                        if (isset($model->user) && !empty($model->user->first_name)) {
                            ...
                            return ' <a  href="javascript:void(0)" onClick="showPopup(\'' . $model->user->id . '\')" style="margin-bottom:5px">' . $model->user->alias .  ' (' . $username . ')</a>';
                        }
                    }
                ],
                ...
                [
                    ...
                    'buttons' => [
                        ...
                        /*'migrate' => function ($url, $model) {
                            $view = isset($_GET['view']) && $_GET['view'] != '' ? '&view=' . $_GET['view'] : '';
                            return Html::a('<span class="glyphicon glyphicon-arrow-up"></span>', Yii::$app->urlManager->createUrl(['sales/lead/migrateone', 'id' => $model->id . $view]), [
...
<input type="hidden" name="multiple_del" value="true">'.Html::a ( '<i class="glyphicon glyphicon-plus"></i> '.Yii::t ( 'app', 'Add' ), [
                    ...
                    'before' =>
                    '<form action="/livecrm/web/index.php?r=sales/lead/migratemulti"  method="post" name="frm">
                                <input type="hidden" name="_csrf" value="' . $csrf . '">                              
                                <input type="hidden" name="multiple_migrate" value="true">' . ' <a href="javascript:void(0)" onClick="all_del()" class="btn btn-info  btn-sm"><i class="glyphicon glyphicon-upload"></i> ' . Yii::t('app', 'Migrar Seleccionados') . '</a>',
