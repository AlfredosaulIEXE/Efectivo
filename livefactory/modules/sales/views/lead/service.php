...
$this->title = 'Leads Atención a Clientes';
                ...
                try {
                    ...
                    echo GridView::widget([
                        ...
                        'columns' => [
                            ...
                            [
                                'attribute' => 'progress',
                                'label' => 'Perfil completo al',
                                'width' => '10%',
                                'format' => 'raw',
                                'value' => function ($model) {
                                    $barClass = $model->progress <= 25 ? 'bg-danger' : ($model->progress <= 50 ? 'bg-warning' : ($model->progress <= 75 ? 'bg-info' : ''));

                                    return '<div class="progress" style="display: inline-block ;width: 150px; margin-bottom: 0">
                        <div class="progress-bar progress-bar-striped '.$barClass.'" role="progressbar"
                            ...
                            [
                                ...
                                'format' => 'raw',
                                'width' => '5%',
                                'filter' => LeadStatus::$_customer_service_status,
                                'filterWidgetOptions' => [
                                    'options' => [
                                        'placeholder' => Yii::t('app', 'All...')
                                    ],
                                ...
                                'value' => function ($model, $key, $index, $widget) {
                                    return LeadStatus::getServiceStatusName($model->service_status_id);
                                }
                            ],
                            ...
                            [
                                ...
                                'width' => '10%',
                                'filter' => ArrayHelper::map(User::find()->join('LEFT JOIN', 'auth_assignment', 'auth_assignment.user_id = tbl_user.id')
                                    ->where("auth_assignment.item_name = 'Customer.Service' OR auth_assignment.item_name = 'Customer.Director'")
                                    ->andWhere($filter && $filter['office_id'] ? 'office_id = ' . $filter['office_id'] : '1')
                                    ->asArray()->all(), 'id', function ($user, $defaultValue) {
                                ...
                                'value' => function ($model, $key, $index, $widget) {
                                    if ($model->service_owner_id) {
                                        $user = User::findOne($model->service_owner_id);
                                        $username = $user->alias ? $user->alias : $user->email;
                                        return ' <a  href="javascript:void(0)" onClick="showPopup(\'' . $user->id . '\')" style="margin-bottom:5px">' . $user->first_name . ' ' . $user->last_name . ' (' . $username . ')</a>';
                                    }
