...
function valide(){
	...
	$('#w0').submit(function(e){
		error='';
			$('[data-validation="required"]').each(function(index, element) {
				...
				$('[data-valid-max-paid="required"]').each(function(){
					Remove_Error($(this));
				if($(this).val() > $('.grand_total').val()){
					error+=Add_Error($(this),'Paid amount can not exceed grand total!');
				}else{
					Remove_Error($(this));
				}
			});
	...
	$('[data-valid-max-paid="required"]').blur(function(){
		Remove_Error($(this));
		if($(this).val() > $('.grand_total').val()){
			error+=Add_Error($(this),'Paid amount can not exceed grand total!');
			event.preventDefault();
...
function autoComplate(){
	...
	$(function(){
		$('#invoice-invoice_number').blur(function(){
			var that = $(this);
			Remove_Error($(that));
			$.post('index.php?r=invoice/invoice/ajax-duplicate-check-insert-case&code='+escape($(this).val())+'&id='+escape('<?=$model->id?>'),function(r){
				if(r > 0){
					Add_Error($(that),'This Invoice Code is already Taken!');
