...
$payment = Payment::find()->where(['entity_id' => $_REQUEST['id']] )->all();
$total = 0;
    ...
    'columns' => [
        ...
        [
            ...
            'value' => function ($model) {
                ...
                $status = $model->origin == 1  ? '' : '<label class="label label-'.$classes[$model->status].'">'.Payment::paymentStatus($model->status).'</label>';
                return  $status;
            }
                ...
                'update' => function ($url, $model) {
                    if (Yii::$app->user->can('Payment.Update')) {
                        return "<form name='frm_payment" . $model->id . "' action='" . 'index.php?r=' . $_REQUEST['r'] . '&id=' . $_REQUEST['id'] . "&payment_edit=" . $model->id . "' method='post' style='display:inline'><input type='hidden' value='$csrf' name='_csrf'>
                                    ...
                                    <a href='#' onClick='document.forms.frm_payment" . $model->id . ".submit()' title='" . Yii::t('app', 'Edit') . "' target='_parent'><span class='glyphicon glyphicon-pencil'></span></a></form>";
                    } else {
                        return '';
                    }
                },
