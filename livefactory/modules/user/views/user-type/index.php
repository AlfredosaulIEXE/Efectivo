 ...
 * @var common\models\search\UserType $searchModel
 */
...
<?php
	if(!empty($_GET['added'])){?>
		<div class="alert alert-success"><?=Yii::t('app', 'User Type is Added')?> </div>
<?php	}
?>
<script src="../../vendor/bower/jquery/dist/jquery.js"></script>
<script>
	/*$(document).ready(function(e) {
       $('td:contains("Customer")').parent().children('td:last').text('');
    });*/
</script>
...
//            'updated_at', 
            ...
            [
				...
				'delete' => function ($url, $model) {
					if($model->type =='Customer' || $model->type =='Employee' || $model->type == 'Sales'){
						return '';
					}else{
                                   /* return Html::a('<span class="glyphicon glyphicon-pencil"></span>', Yii::$app->urlManager->createUrl(['user/user-type/view','id' => $model->id,'edit'=>'t']), [
                                                    'title' => Yii::t('app', 'Edit'),
                                                  ]);
					}*/
										return Html::a('<span class="glyphicon glyphicon-trash"></span>', Yii::$app->urlManager->createUrl(['user/user-type/delete','id' => $model->id]), [
				...
				'view' => function($url,$model){
					...
					return '';	}else{
                                    ...
                                    return Html::a('<span class="glyphicon glyphicon-eye-open"></span>', Yii::$app->urlManager->createUrl(['user/user-type/view','id' => $model->id,'edit'=>'t']), [
                                                    'title' => Yii::t('app', 'Edit'),
					]);}
				...
				'defaultValue' => function ($url, $model) {
					if(\livefactory\models\DefaultValueModule::checkDefaultValue('user_type',$model->id)){
						return Html::a('<span class="fa fa-eraser"></span>', Yii::$app->urlManager->createUrl(['user/user-type/index','del_id' => $model->id]), [
                                                    'title' => Yii::t('app', 'Delete Default'),
                                                  ]);
					}else{
						return Html::a('<span class="fa fa-tag"></span>', Yii::$app->urlManager->createUrl(['user/user-type/index','id' => $model->id]), [
